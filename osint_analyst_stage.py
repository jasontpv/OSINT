...
class CrossReferenceEngine:
    ...
    def extract_facts_from_source(self, source_data: Dict, tool_name: str) -> List[Fact]:
        """Extract structured facts from raw tool output"""
        extracted = []
        
        # Email extraction
        emails = self._extract_field(source_data, ['email', 'emails'])
        for email in emails:
            extracted.append(Fact(
                fact_type='email',
                value=email,
                sources=[tool_name],
                confidence_score=90.0 if len(emails) == 1 else 85.0
            ))
        
        # Username extraction
        usernames = self._extract_field(source_data, ['username', 'usernames', 'handle'])
        for username in usernames:
            extracted.append(Fact(
                fact_type='username',
                value=username,
                sources=[tool_name],
                confidence_score=95.0 if len(usernames) == 1 else 90.0
            ))
        
        # Location extraction
        locations = self._extract_field(source_data, ['location', 'city', 'country'])
        ...
        # Company extraction
        companies = self._extract_field(source_data, ['company', 'organization'])
        for company in companies:
            extracted.append(Fact(
                fact_type='company',
                value=company,
                sources=[tool_name],
                confidence_score=95.0 if len(companies) == 1 else 90.0
            ))
        
        # IP addresses extraction
        ips = self._extract_field(source_data, ['ip', 'ips', 'network_ips'])
        for ip in ips:
            extracted.append(Fact(
                fact_type='ip_addresses',
                value=ip,
                sources=[tool_name],
                confidence_score=98.0
            ))
        
        # Bio/keyword extraction (requires LLM processing)
        bio_text = self._extract_field(source_data, ['bio', 'about', 'description'])
        if bio_text:
            extracted.append(Fact(
                fact_type='bio_keywords',
                value=bio_text[0],
                sources=[tool_name],
                confidence_score=75.0,
                context={"raw_bio": " ".join(bio_text)}
            ))
        
        return extracted
    
    def _extract_field(self, source_data: Dict, field_patterns: List[str]) -> List[str]:
...
Format your response as JSON with keys: "skills", "interests", "locations", "organizations"""
                    ...
                    'role': 'user',
                    'content': bio_text[:500]  # Truncate for context window
                }]
            )
...
def main():
    """Main entry point for analyst stage"""
    import argparse
    ...
    # Load input data (auto-detect or use specified file)
    if args.input:
        raw_data = load_harvest_results(args.input)
    else:
        # Auto-detect latest harvested result
        ...
        if not files:
            print("❌ No harvested results found. Run HARVEST stage first.")
            return
