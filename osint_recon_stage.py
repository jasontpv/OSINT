...
class ReconEngine:
    ...
    def __init__(self):
        ...
        self.dork_templates = {
            EntityType.PERSON: [
                ...
                f'"{entity}" (twitter|X) OR (instagram|facebook)',
                f'{entity} filetype:pdf OR filetype:pptx',
                f'site:github.com "{entity}" -repo:',
                f'"{entity}" (bio OR profile) site:*',
                f'"{entity}" -site:{domain}',  # Exclude known sites for fresh finds
                f'intext:"{entity}" (resume OR cv OR "curriculum")',
            ],
            EntityType.COMPANY: [
                'site:linkedin.com/company "{entity}" employees',
                f'"{entity}" OR "{alias1}" site:*',
                f'site:crunchbase.com {entity} funding',
                f'{entity} (API OR "developer" OR endpoint)',
                f'"{entity}" (integration OR "partner")',
                f'site:github.com/orgs/ {entity}',
                f'"{entity}" filetype:json OR filetype:yml',  # Config/API files
                f'intext:"{entity}" AND ("API key" OR "secret") -site:*,com',
            ],
        }
        ...
        self.api_endpoints = {
            ...
            'twitter_x': {
                ...
                'query_template': f'q="{entity}"&max_results={limit}',
                'auth_header': 'Authorization: Bearer {token}'
            },
            'github': {
                'base': 'https://api.github.com/search/users',
                'query_template': f'q={entity}&per_page={limit}',
                'auth_header': 'Authorization: token {token}'
            },
            'facebook': {
                'base': 'https://graph.facebook.com/search',
                'query_template': f'q={entity}&type=user&limit={limit}',
                'auth_header': 'Access Token {token}'
            },
            'instagram': {
                'base': 'https://api.instagram.com/v1/users/search',
                'query_template': f'q={entity}&count={limit}',
                'auth_header': 'Authorization: Bearer {token}'
            },
        }

    def detect_entity_types(self, input_text: str) -> Tuple[List[EntityType], Dict[str, float]]:
        """
        ...
        return detected_types or [EntityType.MIXED.value], detections
    ...
    def generate_dorks(self, entity_types: List[str], entities: Dict) -> List[Dict]:
        ...
        for etype in entity_types:
            if etype == EntityType.PERSON.value and entities['persons']:
                ...
                base_dorks = [
                    f'site:{domain} "{person}" -intitle:job',
                ...
                company_dorks = [
                    ...
                    f'{company} (API OR "developer") -site:{domain}',
    ...
    def execute_recon(self, user_input: str) -> ReconOutput:
        ...
        return ReconOutput(
            queries=[d['query'] for d in dorks_filtered],
