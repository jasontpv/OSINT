...
Uses Serper.dev for Google searches and Scrapingant.com for HTML scraping.
    ...
    async def scrape_url(self, url: str, max_retries: int = 3) -> ScrapedContent:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if status == 200:
                    ...
                    return ScrapedContent(
                        ...
                        status_code=status,
                        html_content=data,
                        text_content="",  # Text extraction can be done downstream
                    ...
                    raise ScrapingantTimeout(f"HTTP {status}")
                ...
                continue
            except ScrapingantTimeout as e:
                error_msg = str(e)
            ...
            if attempt < max_retries - 1:
                backoff = (2 ** (attempt + 1)) * 0.5 + 0.1  # Exponential backoff
                print(f"⚠️ Attempt {attempt + 1} failed. Retrying in {backoff:.1f}s...")
...
class SerperClient:
    ...
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.rate_limiter = RateLimiter(rate=10.0)  # 10 req/s for search
    ...
    async def execute_search(self, query: str, provider: SearchProvider = SearchProvider.SERPER) -> Dict[str, Any]:
        ...
        Args:
            query: Search query (Google Dork)
...
async def execute_osint_harvest(
    ...
    Returns:
        HarvestOutput with all search results
    """
    ...
    scrapes: List[ScrapedContent] = []
    
    async def execute_single_dork(dork: str):
        """Execute a single dork with concurrency control"""
    ...
    # Execute all dorks concurrently with WIP limit
    tasks = [execute_single_dork(dork) for dork in dorks]
